...
public class DebugUtils {
        ...
        for (IContainer container : region.getSubBlocks()) {
            if (container instanceof IRegion) {
                ...
                if (printInsns && (container instanceof IBlock)) {
                    printInsns(mth, indent, (IBlock) container);
                }
            }
        }
    }
    ...
    private static void printInsns(MethodNode mth, String indent, IBlock block) {
        for (InsnNode insn : block.getInstructions()) {
            try {
                InsnGen ig = new InsnGen(MethodGen.getFallbackMethodGen(mth), true);
                CodeWriter code = new CodeWriter();
                ig.makeInsn(insn, code);
                LOG.debug("{} - {}", indent, code.toString().substring(CodeWriter.NL.length()));
