...
public class BlockFinallyExtract extends AbstractVisitor {
    ...
    private static boolean processExceptionHandler(MethodNode mth, ExceptionHandler excHandler) {
        ...
        for (BlockNode excBlock : excHandler.getBlocks()) {
            ...
            int size = insns.size();
            if (excHandler.isCatchAll() && size != 0 && ((InsnNode) insns.get(size - 1)).getType() == InsnType.THROW) {
                reThrowRemoved = true;
                insns.remove(size - 1);
            }
        }
        if (reThrowRemoved && noExitNode && extractFinally(mth, excHandler)) {
            return true;
        }
        if (countInstructions(excHandler) == 0 && reThrowRemoved && noExitNode) {
    ...
    private static boolean extractFinally(MethodNode mth, ExceptionHandler handler) {
        ...
        for (BlockNode block : handler.getBlocks()) {
            ...
            if (!insns.isEmpty()) {
                if (((InsnNode) insns.get(0)).getType() != InsnType.MOVE_EXCEPTION) {
                    blocks.add(block);
                }
                bs.set(block.getId());
            }
            ...
            for (ExceptionHandler otherHandler : tryBlock.getHandlers()) {
                if (otherHandler != handler) {
                    for (BlockNode hb : otherHandler.getBlocks()) {
                        removeInfo = removeInsns(mth, hb, blocks, bs);
                        if (removeInfo != null) {
        ...
        for (ExceptionHandler otherHandler2 : tryBlock.getHandlers()) {
            SplitterBlockAttr splitterAttr = (SplitterBlockAttr) otherHandler2.getHandlerBlock().get(AType.SPLITTER_BLOCK);
        ...
        for (BlockNode splitter : splitters) {
            for (BlockNode block2 : BlockUtils.collectBlocksDominatedBy(splitter, (BlockNode) splitter.getCleanSuccessors().get(0))) {
                if (!bs.get(block2.getId())) {
                    removeInfo = removeInsns(mth, block2, blocks, bs);
                    if (removeInfo != null) {
                        removes.add(removeInfo);
                        removed = true;
                        break;
        ...
        for (BlocksRemoveInfo removeInfo2 : removes) {
            if (!applyRemove(mth, removeInfo2)) {
                return false;
            }
        }
        LiveVarAnalysis liveVarAnalysis = null;
        ...
        if (me != null && me.getType() == InsnType.MOVE_EXCEPTION) {
            ...
            if (!handlerBlock.getCleanSuccessors().isEmpty()) {
                liveVarAnalysis = new LiveVarAnalysis(mth);
                liveVarAnalysis.runAnalysis();
                RegisterArg resArg = me.getResult();
                if (liveVarAnalysis.isLive((BlockNode) handlerBlock.getCleanSuccessors().get(0), resArg.getRegNum())) {
        ...
        if (runReMap) {
            if (liveVarAnalysis == null) {
                liveVarAnalysis = new LiveVarAnalysis(mth);
                liveVarAnalysis.runAnalysis();
            }
            performVariablesReMap(mth, removes, laBefore, liveVarAnalysis);
        }
    ...
    private static void performVariablesReMap(MethodNode mth, List<BlocksRemoveInfo> removes, LiveVarAnalysis laBefore, LiveVarAnalysis laAfter) {
        ...
        for (BlocksRemoveInfo removeInfo : removes) {
            ...
            if (!(removeInfo.getRegMap().isEmpty() || insertBlock == null)) {
                for (Entry<RegisterArg, RegisterArg> entry : removeInfo.getRegMap().entrySet()) {
                    RegisterArg fromReg = (RegisterArg) entry.getKey();
                    RegisterArg toReg = (RegisterArg) entry.getValue();
                    ...
                    if (!processed.get(fromRegNum)) {
                        ...
                        if (laAfter.isLive(insertBlock, toRegNum) && liveFromBefore) {
    ...
    private static void injectInsn(MethodNode mth, BlockNode insertBlock, InsnNode insn) {
        ...
        if (insertBlock.getInstructions().isEmpty()) {
            ...
            ((BlockNode) splitBlock(mth, insertBlock, 0).getPredecessors().get(0)).getInstructions().add(insn);
        }
    }
    ...
    private static BlocksRemoveInfo removeInsns(MethodNode mth, BlockNode remBlock, List<BlockNode> blocks, BitSet bs) {
        ...
        BlocksRemoveInfo removeInfo = checkFromFirstBlock(remBlock, (BlockNode) blocks.get(0), bs);
        if (removeInfo == null) {
            return null;
        }
        Set<BlocksPair> outs = removeInfo.getOuts();
        if (outs.size() == 1 || mergeReturns(mth, outs)) {
            return removeInfo;
        }
    ...
    private static boolean mergeReturns(MethodNode mth, Set<BlocksPair> outs) {
        ...
        for (BlocksPair outPair : outs) {
            if (!outPair.getFirst().isReturnBlock()) {
                allReturns = false;
            }
        ...
        while (it.hasNext()) {
            BlockNode returnBlock = ((BlocksPair) it.next()).getFirst();
    ...
    private static boolean checkInsns(List<InsnNode> remInsns, List<InsnNode> startInsns, int delta, @Nullable BlocksRemoveInfo removeInfo) {
        for (int i = startInsns.size() - 1; i >= 0; i--) {
            if (!sameInsns((InsnNode) remInsns.get(delta + i), (InsnNode) startInsns.get(i), removeInfo)) {
                return false;
            }
        }
        return true;
    ...
    private static boolean checkBlocksTree(BlockNode remBlock, BlockNode startBlock, BlocksRemoveInfo removeInfo, BitSet bs) {
        ...
        for (int i = 0; i < baseCS.size(); i++) {
            BlockNode sBlock = (BlockNode) baseCS.get(i);
            BlockNode rBlock = (BlockNode) remCS.get(i);
            if (!bs.get(sBlock.getId())) {
                removeInfo.getOuts().add(new BlocksPair(rBlock, sBlock));
            } else if (removeInfo.getEndSplitIndex() != 0 || !checkBlocksTree(rBlock, sBlock, removeInfo, bs)) {
                return false;
            }
        }
        return true;
    ...
    private static boolean sameBlocks(BlockNode remBlock, BlockNode finallyBlock, BlocksRemoveInfo removeInfo) {
        ...
        for (int i = 0; i < size; i++) {
            if (!sameInsns((InsnNode) first.get(i), (InsnNode) second.get(i), removeInfo)) {
                return false;
            }
        }
        if (first.size() > second.size()) {
        ...
        for (int i = 0; i < remInsn.getArgsCount(); i++) {
            ...
            if (removeInfo != null && fArg.isRegister()) {
                ...
                if (remReg.getRegNum() != fReg.getRegNum()) {
                    RegisterArg mapReg = (RegisterArg) removeInfo.getRegMap().get(remArg);
                    if (mapReg == null) {
    ...
    private static boolean applyRemove(MethodNode mth, BlocksRemoveInfo removeInfo) {
        ...
        if (remBlock.getPredecessors().size() != 1) {
            LOG.warn("Finally extract failed: remBlock pred: {}, {}, method: {}", new Object[]{remBlock, remBlock.getPredecessors(), mth});
            return false;
        }
        Iterator i$;
        removeInfo.setStartPredecessor((BlockNode) remBlock.getPredecessors().get(0));
        ...
        if (endSplitIndex > 0) {
            BlockNode newOut = splitBlock(mth, removeInfo.getEnd().getFirst(), endSplitIndex);
            for (BlockNode s : newOut.getSuccessors()) {
                BlocksPair replaceOut = null;
                Iterator<BlocksPair> it = removeInfo.getOuts().iterator();
                while (it.hasNext()) {
                    BlocksPair outPair = (BlocksPair) it.next();
                    if (outPair.getFirst().equals(s)) {
                        it.remove();
                        BlocksPair blocksPair = new BlocksPair(newOut, outPair.getSecond());
                        break;
                    }
                }
                if (replaceOut != null) {
        ...
        BlocksPair out = (BlocksPair) removeInfo.getOuts().iterator().next();
        BlockNode rOut = out.getFirst();
        BlockNode sOut = out.getSecond();
        List<BlockNode> filtPreds = BlockUtils.filterPredecessors(sOut);
        if (filtPreds.size() > 1) {
            BlockNode newPred = BlockSplitter.insertBlockBetween(mth, (BlockNode) sOut.getPredecessors().get(0), sOut);
            i$ = new ArrayList(sOut.getPredecessors()).iterator();
            while (i$.hasNext()) {
                BlockNode predBlock = (BlockNode) i$.next();
                if (predBlock != newPred) {
                    BlockSplitter.removeConnection(predBlock, sOut);
                    BlockSplitter.connect(predBlock, newPred);
                }
            }
            rOut.getPredecessors().clear();
            addIgnoredEdge(newPred, rOut);
            BlockSplitter.connect(newPred, rOut);
        } else if (filtPreds.size() == 1) {
            BlockNode pred = (BlockNode) filtPreds.get(0);
            BlockNode repl = removeInfo.getBySecond(pred);
            if (repl == null) {
                LOG.error("Block not found by {}, in {}, method: {}", new Object[]{pred, removeInfo, mth});
                return false;
            }
            BlockSplitter.removeConnection(pred, rOut);
            addIgnoredEdge(repl, rOut);
            BlockSplitter.connect(repl, rOut);
        } else {
            throw new JadxRuntimeException("Finally extract failed, unexpected preds: " + filtPreds + " for " + sOut + ", method: " + mth);
        }
        i$ = new ArrayList(remBlock.getPredecessors()).iterator();
        while (i$.hasNext()) {
            BlockNode middle = BlockSplitter.insertBlockBetween(mth, (BlockNode) i$.next(), remBlock);
            BlockSplitter.removeConnection(middle, remBlock);
            BlockSplitter.connect(middle, startBlock);
            addIgnoredEdge(middle, startBlock);
            BlockSplitter.connect(middle, rOut);
        }
        markForRemove(mth, remBlock);
        for (BlocksPair pair : removeInfo.getProcessed()) {
            markForRemove(mth, pair.getFirst());
            pair.getSecond().updateCleanSuccessors();
        }
        return true;
    }
    ...
    private static BlockNode splitBlock(MethodNode mth, BlockNode block, int splitIndex) {
        ...
        Iterator i$ = new ArrayList(block.getSuccessors()).iterator();
        while (i$.hasNext()) {
            BlockNode s = (BlockNode) i$.next();
            BlockSplitter.removeConnection(block, s);
            BlockSplitter.connect(newBlock, s);
        }
        ...
        for (int i = splitIndex; i < size; i++) {
            InsnNode insnNode = (InsnNode) insns.get(i);
        ...
        for (InsnNode insnNode2 : newBlock.getInstructions()) {
            insnNode2.remove(AFlag.SKIP);
        }
        return newBlock;
    }
    ...
    private static void addIgnoredEdge(BlockNode from, BlockNode toBlock) {
        IgnoreEdgeAttr edgeAttr = (IgnoreEdgeAttr) from.get(AType.IGNORE_EDGE);
        if (edgeAttr == null) {
    ...
    private static int countInstructions(ExceptionHandler excHandler) {
        ...
        for (BlockNode excBlock : excHandler.getBlocks()) {
            ...
            if (!list.isEmpty() && ((InsnNode) list.get(0)).getType() == InsnType.MOVE_EXCEPTION) {
                totalSize--;
            }
            totalSize += list.size();
        }
    ...
    private static void mergeReturnBlocks(MethodNode mth) {
        BlockNode pred = getFinallyOutBlock(mth.getExitBlocks());
        if (pred != null) {
                        ...
                        for (BlockNode mb : merge) {
                            if (mb != origReturnBlock) {
                                for (BlockNode remPred : mb.getPredecessors()) {
                                    BlockSplitter.connect(remPred, origReturnBlock);
                                }
    ...
    private static void mergeSyntheticPredecessors(MethodNode mth, BlockNode block) {
        ...
        if (preds.size() >= 2) {
            ...
            if (commonBlock != null) {
                ...
                for (BlockNode predBlock2 : preds) {
                    if (mergeBlock == null) {
                        mergeBlock = predBlock2;
                    } else {
                        for (BlockNode remPred : predBlock2.getPredecessors()) {
                            BlockSplitter.connect(remPred, mergeBlock);
                        }
                        markForRemove(mth, predBlock2);
                    }
                }
            }
        }
    ...
    private static BlockNode getFinallyOutBlock(List<BlockNode> exitBlocks) {
        for (BlockNode exitBlock : exitBlocks) {
            for (BlockNode exitPred : exitBlock.getPredecessors()) {
                IgnoreEdgeAttr edgeAttr = (IgnoreEdgeAttr) exitPred.get(AType.IGNORE_EDGE);
