...
public class SSATransform extends AbstractVisitor {
    ...
    private static void process(MethodNode mth) {
        ...
        int k = 0;
        while (true) {
            boolean repeatFix = fixUselessPhi(mth);
            int k2 = k + 1;
            if (k > 50) {
    ...
    public static PhiInsn addPhi(MethodNode mth, BlockNode block, int regNum) {
        PhiListAttr phiList = (PhiListAttr) block.get(AType.PHI_LIST);
        if (phiList == null) {
    ...
    private static SSAVar newSSAVar(MethodNode mth, int[] versions, RegisterArg arg, int regNum) {
        ...
        versions[regNum] = version + 1;
        return mth.makeNewSVar(regNum, version, arg);
    }

    private static void initPhiInEnterBlock(SSAVar[] vars, BlockNode enterBlock) {
        PhiListAttr phiList = (PhiListAttr) enterBlock.get(AType.PHI_LIST);
        if (phiList != null) {
            for (PhiInsn phiInsn : phiList.getList()) {
                SSAVar var = vars[phiInsn.getResult().getRegNum()];
                if (var != null) {
                    var.use(phiInsn.bindArg(enterBlock));
    ...
    private static void renameVar(MethodNode mth, SSAVar[] vars, int[] vers, BlockNode block) {
        SSAVar[] inputVars = (SSAVar[]) Arrays.copyOf(vars, vars.length);
        ...
        for (BlockNode s : block.getSuccessors()) {
            PhiListAttr phiList = (PhiListAttr) s.get(AType.PHI_LIST);
            if (phiList != null) {
                for (PhiInsn phiInsn : phiList.getList()) {
                    var = vars[phiInsn.getResult().getRegNum()];
                    if (var != null) {
                        var.use(phiInsn.bindArg(block));
                        var.setUsedInPhi(phiInsn);
                    }
                }
            }
    ...
    private static boolean fixUselessPhi(MethodNode mth) {
        ...
        for (BlockNode block : mth.getBasicBlocks()) {
            ...
            if (phiList != null) {
                ...
                while (it.hasNext()) {
                    if (fixPhiWithSameArgs(mth, block, (PhiInsn) it.next())) {
                        it.remove();
                        changed = true;
                    }
                }
    ...
    private static boolean fixPhiWithSameArgs(MethodNode mth, BlockNode block, PhiInsn phi) {
        if (phi.getArgsCount() == 0) {
            ...
            InstructionRemover.remove(mth, block, (InsnNode) phi);
            return true;
        }
        boolean allSame;
        if (phi.getArgsCount() == 1 || isSameArgs(phi)) {
            allSame = true;
        } else {
            allSame = false;
        }
    ...
    private static boolean removePhiList(MethodNode mth, List<PhiInsn> insnToRemove) {
        for (BlockNode block : mth.getBasicBlocks()) {
            ...
            if (phiList != null) {
                ...
                for (InsnNode phiInsn : insnToRemove) {
    ...
    private static boolean inlinePhiInsn(MethodNode mth, BlockNode block, PhiInsn phi) {
        ...
        Iterator i$ = new ArrayList(resVar.getUseList()).iterator();
        while (i$.hasNext()) {
            RegisterArg useArg = (RegisterArg) i$.next();
            InsnNode useInsn = useArg.getParentInsn();
