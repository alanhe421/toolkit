...
import jadx.core.dex.attributes.nodes.EnumMapAttr.KeyValueMap;
...
public class ReSugarCode extends AbstractVisitor {
    ...
    private static InsnNode process(MethodNode mth, List<InsnNode> instructions, int i, InstructionRemover remover) {
        InsnNode insn = (InsnNode) instructions.get(i);
        switch (insn.getType()) {
            case NEW_ARRAY:
    ...
    private static InsnNode processNewArray(MethodNode mth, List<InsnNode> instructions, int i, InstructionRemover remover) {
        ...
        int size = instructions.size();
        if (len <= 0 || i + len >= size || ((InsnNode) instructions.get(i + len)).getType() != InsnType.APUT) {
            return null;
        }
        InsnNode filledArr = new FilledNewArrayNode(newArrayInsn.getArrayType().getArrayElement(), len);
        filledArr.setResult(newArrayInsn.getResult());
        for (int j = 0; j < len; j++) {
            InsnNode put = (InsnNode) instructions.get((i + 1) + j);
    ...
    private static InsnNode processEnumSwitch(MethodNode mth, SwitchNode insn) {
        ...
        KeyValueMap valueMap = getEnumMap(mth, enumMapField);
        if (valueMap == null) {
            return null;
        }
    ...
    private static KeyValueMap getEnumMap(MethodNode mth, FieldNode field) {
        ClassNode syntheticClass = field.getParentClass();
        EnumMapAttr mapAttr = (EnumMapAttr) syntheticClass.get(AType.ENUM_MAP);
        if (mapAttr != null) {
                ...
                if (field != null && enumArg.isInsnWrap()) {
                    ...
                    if (sget instanceof IndexInsnNode) {
                        ...
                        if (index instanceof FieldInfo) {
                            ...
                            if (fieldNode != null) {
                                mapAttr.add(field, Integer.valueOf((int) ((LiteralArg) litArg).getLiteral()), fieldNode);
                            }
                        }
                    }
                }
    ...
    private static void checkAndHideClass(ClassNode cls) {
        for (FieldNode field : cls.getFields()) {
            ...
            if (af.isSynthetic() && af.isStatic() && af.isFinal() && !field.contains(AFlag.DONT_GENERATE)) {
                return;
            }
        }
