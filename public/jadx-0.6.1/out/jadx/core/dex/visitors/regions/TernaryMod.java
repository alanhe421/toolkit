...
public class TernaryMod {
    ...
    static boolean makeTernaryInsn(MethodNode mth, IfRegion ifRegion) {
        ...
        InsnNode t = (InsnNode) tb.getInstructions().get(0);
        InsnNode e = (InsnNode) eb.getInstructions().get(0);
        if (t.getSourceLine() != e.getSourceLine()) {
            if (t.getSourceLine() == 0 || e.getSourceLine() == 0) {
                if (containsTernary(t) || containsTernary(e)) {
                    return false;
                }
            } else if (!checkLineStats(t, e)) {
                return false;
            }
        }
        TernaryInsn ternInsn;
        if (t.getResult() != null && e.getResult() != null) {
            ...
            ternInsn = new TernaryInsn(ifRegion.getCondition(), resArg, InsnArg.wrapArg(t), InsnArg.wrapArg(e));
            ternInsn.setSourceLine(t.getSourceLine());
            header.getInstructions().clear();
            header.getInstructions().add(ternInsn);
            CodeShrinker.shrinkMethod(mth);
            return true;
        } else if (mth.getReturnType().equals(ArgType.VOID) || t.getType() != InsnType.RETURN || e.getType() != InsnType.RETURN) {
            return false;
        } else {
            if (!ifRegion.getParent().replaceSubBlock(ifRegion, header)) {
                return false;
            ...
            ternInsn = new TernaryInsn(ifRegion.getCondition(), null, t.getArg(0), e.getArg(0));
            ternInsn.setSourceLine(t.getSourceLine());
    ...
    private static BlockNode getTernaryInsnBlock(IContainer thenRegion) {
        if (thenRegion instanceof Region) {
            ...
            if (r.getSubBlocks().size() == 1) {
                IContainer container = (IContainer) r.getSubBlocks().get(0);
        ...
        for (InsnArg arg : tPhi.getArguments()) {
            if (arg.isRegister()) {
                ...
                if (assignInsn != null) {
                    ...
                    if (sourceLine != 0) {
                        Integer count = (Integer) map.get(Integer.valueOf(sourceLine));
                        if (count != null) {
                            map.put(Integer.valueOf(sourceLine), Integer.valueOf(count.intValue() + 1));
                        } else {
                            map.put(Integer.valueOf(sourceLine), Integer.valueOf(1));
                        }
                    }
                }
            }
