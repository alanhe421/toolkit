...
public class LoopRegionVisitor extends AbstractVisitor implements IRegionVisitor {
    ...
    private static void processLoopRegion(MethodNode mth, LoopRegion loopRegion) {
        if (!loopRegion.isConditionAtEnd()) {
            ...
            if (condition != null && !checkForIndexedLoop(mth, loopRegion, condition) && checkIterableForEach(mth, loopRegion, condition)) {
            }
        }
    }
    ...
    private static boolean checkForIndexedLoop(MethodNode mth, LoopRegion loopRegion, IfCondition condition) {
        ...
        if (!condition.getRegisterArgs().contains(arg) || arg.getSVar().isUsedInPhi()) {
            return false;
        }
    ...
    private static LoopType checkArrayForEach(MethodNode mth, InsnNode initInsn, InsnNode incrInsn, IfCondition condition) {
        ...
        if (((ArithNode) incrInsn).getOp() != ArithOp.ADD) {
            return null;
        }
        ...
        List<RegisterArg> args = ((RegisterArg) condArg).getSVar().getUseList();
        if (args.size() != 3 || args.get(2) != condArg) {
            return null;
        }
        condArg = (InsnArg) args.get(0);
        InsnNode arrGetInsn = ((RegisterArg) args.get(1)).getParentInsn();
        ...
        if (!len.getArg(0).equals(arrGetInsn.getArg(0))) {
            return null;
        }
    ...
    private static boolean checkIterableForEach(MethodNode mth, LoopRegion loopRegion, IfCondition condition) {
        ...
        RegisterArg iteratorArg = (RegisterArg) condArgs.get(0);
        ...
        InsnNode hasNextCall = ((RegisterArg) useList.get(0)).getParentInsn();
        InsnNode nextCall = ((RegisterArg) useList.get(1)).getParentInsn();
        ...
        if (nextCall.contains(AFlag.WRAPPED)) {
            ...
            } else if (!fixIterableType(mth, iterableArg, iterVar)) {
                return false;
            } else {
    ...
    private static boolean fixIterableType(MethodNode mth, InsnArg iterableArg, RegisterArg iterVar) {
        ...
        if (iterableType.isGeneric()) {
            ...
            if (gType.isGenericType()) {
                ...
                return true;
            } else if (ArgType.isInstanceOf(mth.dex(), gType, varType)) {
                return true;
            } else {
                ...
                LOG.warn("Generic type differs: '{}' and '{}' in {}", new Object[]{gType, varType, mth});
                return false;
            }
        } else if (!iterableArg.isRegister()) {
            return true;
        } else {
            iterableArg.setType(ArgType.generic(iterableType.getObject(), new ArgType[]{varType}));
            return true;
        }
    }
    ...
    private static boolean checkInvoke(InsnNode insn, String declClsFullName, String mthId, int argsCount) {
        ...
        if (declClsFullName == null || callMth.getDeclClass().getFullName().equals(declClsFullName)) {
            return true;
        }
        return false;
    }
    ...
    private static boolean assignOnlyInLoop(MethodNode mth, LoopRegion loopRegion, RegisterArg arg) {
        ...
        for (InsnArg phiArg : ((PhiInsn) assignInsn).getArguments()) {
