...
public class RegionGen extends InsnGen {
    ...
    public void makeRegion(CodeWriter code, IContainer cont) throws CodegenException {
        ...
        } else if (cont instanceof Region) {
    ...
    private void declareVars(CodeWriter code, IContainer cont) {
        DeclareVariablesAttr declVars = (DeclareVariablesAttr) cont.get(AType.DECLARE_VARIABLES);
        if (declVars != null) {
    ...
    private void makeSimpleBlock(IBlock block, CodeWriter code) throws CodegenException {
        ...
        ForceReturnAttr retAttr = (ForceReturnAttr) block.get(AType.FORCE_RETURN);
        if (retAttr != null) {
        ...
        if (header != null) {
            ...
            if (headerInsns.size() > 1) {
                ...
                for (int i = 0; i < last; i++) {
                    makeInsn((InsnNode) headerInsns.get(i), code);
                }
            }
        }
        LoopLabelAttr labelAttr = (LoopLabelAttr) region.getInfo().getStart().get(AType.LOOP_LABEL);
        if (labelAttr != null) {
    ...
    private CodeWriter makeSwitch(SwitchRegion sw, CodeWriter code) throws CodegenException {
        InsnArg arg = ((SwitchNode) sw.getHeader().getInstructions().get(0)).getArg(0);
        ...
        for (int i = 0; i < size; i++) {
            IContainer c = (IContainer) sw.getCases().get(i);
            for (FieldNode k : (List) sw.getKeys().get(i)) {
                code.startLine("case ");
                if (k instanceof FieldNode) {
                    ...
                    if (fn.getParentClass().isEnum()) {
                        ...
                        FieldInitAttr valueAttr = (FieldInitAttr) fn.get(AType.FIELD_INIT);
                        if (!(valueAttr == null || valueAttr.getValue() == null)) {
                            code.add(" /*").add(valueAttr.getValue().toString()).add("*/");
                        }
                    }
                } else if (k instanceof Integer) {
                    code.add(TypeGen.literalToString((long) ((Integer) k).intValue(), arg.getType(), this.mth));
                } else {
    ...
    private void makeTryCatch(TryCatchRegion region, CodeWriter code) throws CodegenException {
        ...
        for (Entry<ExceptionHandler, IContainer> entry : region.getCatchRegions().entrySet()) {
            ExceptionHandler handler = (ExceptionHandler) entry.getKey();
