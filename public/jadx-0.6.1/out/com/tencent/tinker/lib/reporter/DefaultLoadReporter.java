...
public class DefaultLoadReporter implements LoadReporter {
    ...
    public void onLoadResult(File file, int i, long j) {
        TinkerLog.i(TAG, "patch loadReporter onLoadResult: patch load result, path:%s, code:%d, cost:%d", file.getAbsolutePath(), Integer.valueOf(i), Long.valueOf(j));
    }

    public void onLoadException(Throwable th, int i) {
        switch (i) {
            case -5:
                TinkerLog.i(TAG, "patch load parallel dex opt exception: %s", th);
                break;
            case -4:
