...
final class DecodedBitStreamParser {
    ...
    private static void decodeKanjiSegment(BitSource bitSource, StringBuilder stringBuilder, int i) throws FormatException {
        ...
        while (i > 0) {
            int readBits = bitSource.readBits(13);
            readBits = (readBits % Opcodes.AND_LONG_2ADDR) | ((readBits / Opcodes.AND_LONG_2ADDR) << 8);
            if (readBits < 7936) {
                readBits += 33088;
            } else {
                readBits += 49472;
    ...
    private static int parseECIValue(BitSource bitSource) throws FormatException {
        ...
        if ((readBits & 128) == 0) {
            return readBits & Opcodes.NEG_FLOAT;
        }
        if ((readBits & Opcodes.AND_LONG_2ADDR) == 128) {
            return ((readBits & 63) << 8) | bitSource.readBits(8);
        } else if ((readBits & Opcodes.SHL_INT_LIT8) == Opcodes.AND_LONG_2ADDR) {
